...
async def main():
    ...
    async def start(m: types.Message):
        await m.answer("Привет! Я бот для учёта расходов.\n\n" + HELP_TEXT, parse_mode="Markdown")
    ...
    async def undo_cmd(m: types.Message):
        ...
        if not row:
            await m.answer("Нечего удалять — записей нет.")
            return
        ...
        await m.answer(
            f"↩️ Удалил последнюю запись:\n"
            f"{dt_s} — {format_money(float(amount))} — {category}{note_part}"
        )
